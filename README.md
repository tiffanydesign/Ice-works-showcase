...
粒子的字形是 PHENOME LONGEVITY 的逐字母拼写（`PHENOMELONGEVITY`，十六格），见 `components/ring/ascii.js`。

着色器按密度索引这个字符集：第 0 格用在粒子场最外缘，最后一格贴着卡片边缘。所以这个顺序决定的是**哪个字母出现在离卡片多远的位置**，画面上并不会读出品牌名——`P` 是最外缘最淡的小点，`Y` 是贴着卡片最重的一笔。

字母本身的墨量跨度只有原符号集的五分之一，且品牌顺序下轻重完全不递增，所以九倍的墨量梯度全靠**每个字形单独求解字号**来实现（44px–150px，字格 160px），启动时按当前平台的等宽字体实测反解。这也是顺序可以自由排成一个单词的前提。
