...
- **Particle Ramp** (700, bold monospace, 44px to 150px in a 160px cell): `PHENOMELONGEVITY`, the brand spelled letter by letter, carrying the entry assembly and the seed-card halo. This is the role where type is read as weight rather than as language: the shader indexes the set by density, cell 0 at the faint outer edge of a field and the last cell against the card. The order is therefore a distance mapping, not a readable word — P is the faintest speck, Y the boldest mark — and the ramp itself is carried by drawing each letter at its own size. See The Particle Ramp Rule.
- **Particle Word** (700, bold monospace, one 160px size in a 160px cell): the same letters on a second atlas row, at a single size, for the hover halo. Here the type _is_ language: the shader indexes it by the cell's own column, so `PHENOME` — the head of the ramp string — reads left to right and tiles across the halo. One size because that is what text is; the shading the ramp used to carry is carried instead by particle density and the distance falloff. See The Particle Word Rule.
...
**The Visible Band Rule.** Particles crowd the near-card end of every falloff, so the last few cells of the ramp are most of what anyone sees. In a ramp-indexed field, letter order is a distance mapping before it is a sequence: whatever sits in those cells sets the character of the whole field. The brand spelling puts `V I T Y` there and reads as thin vertical strokes, which was chosen knowingly over an ink-sorted set that would put `G O M N` there instead. Judge any reordering on that band, not on the atlas.

**The Particle Word Rule.** A field that spells something indexes the atlas by the cell's column, never by its distance from the card — distance scatters letters radially, which is the whole reason the ramp reads as texture and not as language. Three things travel with that choice. The letters come off the word row, which is drawn at one size, because a word with a per-letter type size is a ransom note. The random thinning that gives every field its grain is lifted toward solid, because a dropped letter in a word reads as a misspelling rather than as texture; the halo still fades out, on the distance falloff alone. And the cell must be large enough to read — seven letters want about half the halo's reach, which is why the hover field's cell is larger than the two ramp fields'. The word tiles cleanly along the top and bottom of a card, where the halo runs with the text; on the flanks the inward drift bends the columns and it reads as texture again. That is expected, not a defect.
